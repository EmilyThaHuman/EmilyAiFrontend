...
export default function setCodeUid(
  code: string,
  anchorUid: string = '',
  path: string = '/mock.tsx'
) {
  ...
  const codeUid = printer.printNode(
...
export function setHtmlCodeUid(
  ...
  if (generatedCodeConfig === GeneratedCodeConfig.HTML_TAILWIND) {
    ...
    if (headMatch) {
      ...
      function htmlRender () {
        return (
    ${headMatch[0].replaceAll(/<!--((.)*)-->/gim, '').replaceAll('$', '')}; // todo: if string include $ out error.
        )
      }
      `;
...
export function setUidAnchorPoint(
  uid: string,
  code: string,
  generatedCodeConfig: GeneratedCodeConfig
) {
