...
export function generateMockNextGeneratedUID(): string | null {
  if (
    ...
    MOCK_NEXT_GENERATED_UIDS_IDX.current <
      MOCK_NEXT_GENERATED_UIDS.current.length
  ) {
    const nextID =
      MOCK_NEXT_GENERATED_UIDS.current[MOCK_NEXT_GENERATED_UIDS_IDX.current];
  ...
  if (mockUID == null) {
    if (possibleStartingValue.length >= 3) {
      ...
      for (let step = 0; step < maxSteps; step++) {
        const possibleUID = possibleStartingValue.substring(
          step * 3,
          (step + 1) * 3
        );
    ...
    throw new Error(
      `Unable to generate a UID from '${possibleStartingValue}'.`
    );
  } else {
    return mockUID;
  }
