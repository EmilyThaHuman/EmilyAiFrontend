...
function wrappedCode(code, framework, theme) {
  if (framework === 'html') {
    return `<html>
  <head>
    <meta charset="UTF-8">
...
async function getRandomPhoto(image, seed) {
  ...
  if (dims) {
    ...
    width = parseInt(match?.[1] ?? '500', 10);
    height = width;
  }

  let results = CACHE.get(image.alt) ?? [];
  let img;
  if (results.length > 0) {
...
function deUnsplashImages(images) {
...
async function unsplashImages(images, seed) {
  const urlPromises = [];
  let idx = 0;
  for (const image of images) {
...
async function parseHTML(html, unsplash = false, seed = Math.random()) {
